...
void MakeTranscode(char *code, char *transcode, List ArrayList, int n) {
	for (int i = 0; i<strlen(code); i++) {
		for (int j = 1; j <= n; j++) {
			if (code[i] == ArrayList[j].word) {
				printf("%s", ArrayList[j].code);
				strcat(transcode, ArrayList[i].code);
			}
		}
	}
	//for(int i=0;i<strlen(transcode);i++)
	//cout<<transcode[i];
	//cout<<endl;
}
void Translate(List ArrayList, char *s1) {
	int i, j, k = 0;
	int num = 0;
...
int main() {
	...
	while (cin >> code) {
		...
		char *transcode = (char*)malloc(200 * sizeof(char));
		for (i = 0; code[i] != '\0'; i++) {
		...
		cout << endl;
		MakeTranscode(code, transcode, ArrayList, j);
		//Translate(ArrayList,transcode);
	}
	return 0;
}
