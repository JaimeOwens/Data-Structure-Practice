...
//获取线性表当前长度
template<class CElemType>int CList<CElemType>::GetSize()
{
    return m_nSize;
}
//获取线性表分配的存储容量
template<class CElemType>int CList<CElemType>::GetLength()
{
    return m_nLength;
}
//检索元素
...
//修改元素
template<class CElemType>void CList<CElemType>::SetAt(int nIndex, CElemType e)
{
    m_pElem[nIndex] = e;
}
//把元素e加到线性表的尾部
template<class CElemType>int CList<CElemType>::AddTail(CElemType e)
{
    return InsertAt(GetSize(), e);
}
//把元素e加到线性表的头部
template<class CElemType>int CList<CElemType>::AddHead(CElemType e)
{
    return InsertAt(0, e);
}
//在指定位置（nIndex）插入元素e，返回索引号
template <class CElemType>int CList<CElemType>::InsertAt(int nIndex, CElemType e)
{
    //索引超出范围
    if (nIndex > GetSize())
    {
...
//从指定位置删除nCounter个元素
template<class CElemType>void CList<CElemType>::RemoveAt(int nIndex, int nCounter)
{
    //索引超出范围或者最后一个元素超出范围
